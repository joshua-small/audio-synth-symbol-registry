...
export function validateAssetRefs(assetRefs) {
  if (
    assetRefs === null ||
    Array.isArray(assetRefs) ||
    typeof assetRefs !== "object" ||
    Object.values(assetRefs).some((value) => typeof value !== "string" || value.length === 0)
  ) {
    throw new Error("Asset map must be a JSON object whose values are non-empty strings.");
  }
  return assetRefs;
}

export function resolveInterchange(ids, registry, assetRefs = {}) {
  validateAssetRefs(assetRefs);
  return ids.map((id) => {
    ...
    if (Object.hasOwn(assetRefs, id)) resolved.asset_ref = assetRefs[id];
    return resolved;
  });
}
...
async function main(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument === "--format" || argument === "--asset-map") {
      if (!argv[index + 1] || argv[index + 1].startsWith("--")) {
        throw new Error(`${argument} requires a value.`);
      }
      if (argument === "--format") format = argv[index + 1];
      else assetMapPath = argv[index + 1];
      index += 1;
    } else {
      ids.push(argument);
    }
  ...
  if (ids.length === 0) throw new Error("Provide at least one canonical registry ID.");

  const assetRefs = assetMapPath
    ? validateAssetRefs(JSON.parse(await readFile(path.resolve(assetMapPath), "utf8")))
    : {};
