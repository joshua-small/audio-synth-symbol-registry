...
test("rejects a README registry release that differs from metadata", () => {
  ...
  assert.equal(result.status, 1, result.stdout);
  assert.match(result.stderr, /README registry release 0\.1\.0 differs from registry metadata 0\.2\.11/);
});
...
test("preserves historical assessments when a newer snapshot is appended", () => {
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /Validated 6 registry record\(s\), 7 assessment set\(s\)/);
});
