...
Scores: 20, 20, 20, 18, 19, and 19 in registry order; unchanged by DA-026.
...
## Evolutionary-history disposition

DA-026 adds EV-210 through EV-221 across a transparent current-DAW editorial frame, current publisher and product-ecosystem documentation, early electrical/audio engineering, synth and broadcast material, 2000s vendor and education snapshots, trade instruction, and practitioner discussion.

The evidence supports long semantic continuity for passed, rejected, and shelf-affected frequency regions, but not one unbroken graphic lineage. Circuit schematics, axis-bearing theoretical or measured plots, labeled physical controls, configured software graphs, cursor and gesture interactions, and compact selectors are distinct communication methods. Similar response topology can follow from shared semantics without copying or exact contour inheritance.

Shelf sources preserve the existing polarity boundary: low/high identifies affected side, while boost/cut is a separate signed state. Axis-bearing graphs can explain configured response but commonly carry gain, frequency, Q, slope, and other state beyond standalone type identity. MIT's source-local Band-stop/Notch wording does not relax DA-020's transfer rule.

DA-026 also preserves a negative result: it found no independently used portable target response glyph or shared character mapping. Its current-DAW frame is editorial rather than statistical, and the corpus remains English-heavy with substantial geographic, language, ephemeral-medium, and archive-survivorship gaps. Bell 1923 remains an acquisition lead rather than a ledger source because page-level bytes were not reproducibly bound.
