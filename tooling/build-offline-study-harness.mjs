...
function renderForcedChoice() {
  ...
  card.innerHTML = stimulusHtml(token) + '<form id="task"><fieldset><legend>' + escapeHtml(instrument.participant_prompt) + '</legend>' + choices + '</fieldset><fieldset><legend>How confident are you in your answer?</legend><div class="confidence">' + confidence + '</div><div class="endpoints"><span>Not at all confident</span><span>Very confident</span></div></fieldset><button type="submit">Save and continue</button></form>';
  document.querySelector("#task").addEventListener("submit", (event) => {
    event.preventDefault();
...
export async function buildOfflineHarness(publicDirectory, outputDirectory, formId) {
  ...
  for (const stimulus of instrument.stimuli) {
    const bytes = await readFile(join(sourceDirectory, stimulus.asset));
