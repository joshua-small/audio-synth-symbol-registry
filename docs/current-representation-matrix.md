...
- **Yes**: the documentation establishes a compact curve or response-form graphic.
- **Contextual only**: a response graph or illustration is documented, but it is embedded in a display, explanatory figure, or signal-flow context.
...
| Evidence | Vendor / product / documented version | Semantic labels observed | Abbreviations observed | Compact response curve | Axes or context | Notable divergence |
| --- | --- | --- | --- | --- | --- | --- |
| [EV-006](https://www.ableton.com/en/live-manual/12/live-audio-effect-reference/) | Ableton / Live / 12 | Low-pass, High-pass, Band-pass, Notch | No abbreviation is used for the four names in the Auto Filter type list | Yes | Contextual UI: the manual documents a Filter Type chooser image and a display that shows the selected filter curve plus modulated curves and output spectrum; axes not established | Uses `Notch` for the rejected-band member and extends the family with Morph, DJ, Comb, Resampling, Notch + LP, and Vowel. The response display carries more state than a standalone semantic mark. |
| [EV-007](https://www.fabfilter.com/help/pro-q/using/bandcontrols) | FabFilter / Pro-Q / 4 | Low Cut, High Cut, Band Pass, Notch; also Bell, Low Shelf, High Shelf, Tilt Shelf, Flat Tilt, All Pass | None established on the cited page | Yes | Contextual UI: a shape button selects a filter form within an interactive EQ display; axes not established | Uses `Low Cut` for high-pass behavior and `High Cut` for low-pass behavior. It treats slope, Q, gain, dynamics, and channel placement as separate mutable properties, so the rendered curve is not purely a type token. |
...
| [EV-012](https://moogmusic-help.freshdesk.com/en/support/solutions/articles/69000877830-moog-matriarch-filter-modes) | Moog Music / Matriarch / version not stated | HIGH PASS, LOW PASS, Band Pass Filter, Notch Filter, band-stop filter | VCF, HP, LP; combined HP/LP modes | Contextual only | Signal-flow images and filter-routing context; no standalone response curve established | Band-pass and notch result from routing HP and LP filters in series or parallel. The source explicitly calls notch `also called a band-stop filter`, but describes a very narrow rejected range, preserving the width-boundary issue recorded elsewhere. |
| [EV-013](https://noiseengineering.us/blogs/loquelic-literitas-the-blog/getting-started-filters/) | Noise Engineering / educational manufacturer article / page version not stated | lowpass, highpass, bandpass, notch; low cut and high cut in the mixing discussion | LP, HP, BP in linked module descriptions | Contextual only | A captioned four-graph explanatory image demonstrates the response categories; graph context is explicit, visible axes not established by the text | Explicitly relates `low cut` to highpass and `high cut` to lowpass. The response forms are teaching graphs, while linked module descriptions use text abbreviations. |
